    ...
    function renderHome() {
            ...
            <section id="home-hero">
                <div class="hero-content" style="display:flex; flex-direction:column; align-items:center; justify-content:center; width:100%; text-align:center;">
                    ...
                    <div style="display:inline-block; position:relative; margin: 12px 0 8px;">
                        <div class="combo-group" style="display:inline-flex; border-radius:60px; overflow:hidden; box-shadow: 0 4px 24px rgba(59,122,110,0.30); transition: transform 0.35s cubic-bezier(0.34, 1.56, 0.64, 1), background 0.35s;">
                            <!-- 左侧：开始测试 -->
                            <button id="startTestBtn" style="border-radius:0; padding:16px 32px; min-width:140px; background:var(--accent); color:#fff; border:none; font-weight:600; font-size:1.05rem; cursor:pointer; letter-spacing:1px; transition: none;">
                                ${startBtnText}
                            </button>
                            <!-- 分割线 -->
                            <div style="width:1px; background:rgba(255,255,255,0.3); flex-shrink:0;"></div>
                            <!-- 右侧：下拉箭头 -->
                            <button id="dropdownToggle" style="border-radius:0; padding:16px 18px; background:var(--accent); color:#fff; border:none; font-weight:600; font-size:1.2rem; cursor:pointer; display:flex; align-items:center; justify-content:center; min-width:52px; transition: none;" aria-label="${getUI('selectQuestionCount')}">
                                ▼
                            </button>
                        </div>
                        <!-- 下拉菜单（作为子元素，absolute定位） -->
        ...
        if (comboGroup) {
            comboGroup.addEventListener('mouseenter', function() {
                ...
                btns.forEach(btn => btn.style.background = 'var(--accent-dark)');
            });
            comboGroup.addEventListener('mouseleave', function() {
                this.style.transform = 'translateY(0) scale(1)';
                const btns = this.querySelectorAll('button');
                btns.forEach(btn => btn.style.background = 'var(--accent)');
            });
        }
