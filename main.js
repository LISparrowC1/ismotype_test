...
(function() {
    ...
    // 六点量表权重映射
    const OPTION_WEIGHTS = [2.5, 1.5, 0.5, -0.5, -1.5, -2.5];
    ...
    function getOptions() {
        const lang = window.LANG || window.LANG_ZH;
        return lang?.ui?.options || ['非常赞同', '赞同', '比较赞同', '比较不赞同', '不赞同', '非常不赞同'];
    }
    ...
    function renderHome() {
        ...
        if (dropdownToggle && dropdownMenu) {
            const newDropdown = dropdownToggle.cloneNode(true);
            ...
            document.addEventListener('click', function(e) {
                if (dropdownMenu.style.display === 'block') {
                    if (!newDropdown.contains(e.target) && !dropdownMenu.contains(e.target)) {
    ...
    function renderQuestion(idx) {
        ...
        const optionLabels = getOptions();
        let opts = '';
        for (let i = 0; i < 6; i++) {
            const sel = answers[idx] === i ? ' selected' : '';
            opts += `<div class="option-btn${sel}" onclick="window.selectAnswer(${idx},${i})"><div class="option-radio"></div><span>${optionLabels[i]}</span></div>`;
        }
    ...
    function calcScores() {
        ...
        const tagScores = new Array(44).fill(0);
        let answered = 0;

        for (let i = 0; i < totalQuestions; i++) {
            if (answers[i] === null) continue;
            const w = OPTION_WEIGHTS[answers[i]];
            const q = QUESTIONS[i];
            if (q?.mbti) {
                for (let d = 0; d < 4; d++) {
                    mbtiScores[d] += (q.mbti[d] || 0) * w;
                }
            }
            if (q?.tags && q.tags.length >= 44) {
                for (let t = 0; t < 44; t++) {
                    tagScores[t] += (q.tags[t] || 0) * w;
                }
            }
            answered++;
        }
        ...
        if (answered === 0) {
            return { mbtiScores: [0, 0, 0, 0], tagScores: new Array(44).fill(0) };
        }
    ...
    function calcIsmScores(tagScores) {
        let norm = 0;
        for (let t = 0; t < 44; t++) norm += tagScores[t] * tagScores[t];
        ...
        for (const [field, isms] of Object.entries(ISMS)) {
            ...
            for (const [name, data] of Object.entries(isms)) {
                let ismNorm = 0;
                for (let t = 0; t < 44; t++) {
                    ismNorm += (data.tags[t] || 0) * (data.tags[t] || 0);
                }
                ismNorm = Math.sqrt(ismNorm) || 1;

                let dot = 0;
                for (let t = 0; t < 44; t++) {
                    dot += userVec[t] * (data.tags[t] || 0) / ismNorm;
                }
                const score = Math.round(((dot + 1) / 2) * 100);
                items.push({
                    name: name,
                    score: Math.max(0, Math.min(100, score)),
                    desc: getIsmDesc(name),
                    icon: data.icon || ''
                });
            }
    ...
    async function init() {
        ...
        console.log('✅ main.js v3.9 加载完成 (六选项 + 44维 + 雷达图)');
    }
